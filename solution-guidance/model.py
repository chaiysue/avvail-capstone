...
def _model_train(df,tag,test=False):
    ...
    baseline_rmse = round(np.sqrt(mean_squared_error(y_test, baseline_pred)))

    # Candidate models: random forest and SVR
    candidates = {}
    param_grid_rf = {
    ...
    candidates["rf"] = GridSearchCV(pipe_rf, param_grid=param_grid_rf, cv=3, n_jobs=-1)
    ...
    pipe_svr = Pipeline(steps=[("scaler", StandardScaler()),
                               ("svr", SVR(kernel="rbf"))])
    candidates["svr"] = GridSearchCV(pipe_svr, param_grid=param_grid_svr, cv=3, n_jobs=-1)
    ...
    for name, estimator in candidates.items():
        ...
        evals.append((rmse, name, estimator))

    eval_rmse, best_name, best_estimator = sorted(evals, key=lambda x: x[0])[0]
    ...
    if test:
        saved_model = os.path.join(MODEL_DIR,
                                   "test-{}-{}-{}.joblib".format(tag,model_name,best_name))
        print("... saving test version of model: {}".format(saved_model))
    else:
        saved_model = os.path.join(MODEL_DIR,
                                   "sl-{}-{}-{}.joblib".format(tag,model_name,best_name))
        print("... saving model: {}".format(saved_model))
        
    joblib.dump(best_estimator,saved_model)
    ...
    update_train_log(
        ...
        {'rmse':eval_rmse, 'baseline_rmse': baseline_rmse, 'selected_model': best_name},
        runtime,
        MODEL_VERSION,
        MODEL_VERSION_NOTE,
        test=test
